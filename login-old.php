...
{   
    ...
    $SQLSt = "DELETE FROM registeraccess WHERE ip_numer = '".$ipaddress."'";
    ...
    $loginquery=$mysqli->query("select * from registeraccess where ip_numer='".$ipaddress."'") ; 
    $data=mysqli_fetch_assoc($loginquery);
    // Si no ha entrado antes lo inserto
    if ($data==NULL){
        //significa que esa Ip no existe.
        $SQLSt = "INSERT INTO registeraccess (ip_numer,access_number,site) VALUES ('".$ipaddress."',1,'login')";
        ...
        if($data['access_number']==3){
            ...
            // old function    $answermail=sendemailadmin($ipaddress);
            /* Hasta Aqui */
                $to = 'freedom8604@hotmail.com'; // Correo del administrador donde llegarà el correo cuando una IP se bloquee
                $sub = "Bloked IP";
            
                $cuerpo="La IP: ".$ipaddress." ha intentado registrarse en tres oportunidades en el Sistema de Fragancias Oasis<br><br>";
                $cuerpo.="Esta IP  ha sido bloqueada por el sistema. En caso que necesite desbloquearla entre en el administrador.<br>";
                $cuerpo.="Este correo es envia automáticamente por nuestro sistema, por favor, no responda ni reenvíe mensajes a esta dirección"."<br>";
            ...
            $SQLSt=$mysqli->query("update registeraccess set access_number = access_number+1 WHERE ip_numer = '".$ipaddress."'");
            header("Location: login_failed.php");
        }
    }

...
function invoice(a,b,c,d,e,f){
    ...
    var c= document.getElementById("subtotal").value=a*b;
     var d=document.getElementById("salestax").value= document.getElementById("subtotal").value * <?php echo $data['sales_tax']; ?>;
...
document.getElementsByName("total<?php echo $counter; ?>")[0].value= document.getElementById("quantity<?php echo $counter; ?>").value * document.getElementById("price<?php echo $counter; ?>").value;
    document.getElementById("subtotal").value = Number(document.getElementById("subtotal").value) + Number(document.getElementsByName("total<?php echo $counter; ?>")[0].value ) 
     ...
     document.getElementById("salestax").value= document.getElementById("subtotal").value * <?php echo $data['sales_tax']; ?>; 
...
<div id="logoImage" style="position:absolute;margin:20px 0 0 150px; ">
  <img src="<?php echo $company['logo']; ?>" width="110"></img>
   ...
   <form  id="myform" action="edit_customer_estimate.php" method="post" onsubmit="changeid()">
...
<tr>
    <?php 
    $query=$mysqli->query("select id,quantity,description,price,total,col_total,subtotal,salestax,total_balance from invoice where invoice_id='$id'");
    
    while($data1=mysqli_fetch_assoc($query)){ ?>
        
        
        
        <td><input name="description[]" value="<?php echo $data1['description'];?>" id="description" ><select id="desc" name="desc_selector" onchange="estimate(this)"> <?php echo get_items(); ?> </select></td>
    <td><input name="quantity[]" id="quantity" value="<?php 
    $number = preg_replace("/[^0-9]/", '', $data1['quantity']);
echo $number;?>" onkeyup="invoice(this)">&nbsp;&nbsp;<select name="quantity_type[]"><option value="SF">SF</option><option value="RE">RE</option></select></td>
	
        <td><input name="price[]" id="price" value="<?php echo $data1['price'];?>" onkeyup="invoice(this)" autocomplete="off"></td>
        <td><input name="total" id="total" value="<?php echo $data1['col_total'];?>" autocomplete="off" ></td>
    ...
    <table class="table">
        <tr>
            ...
            <td></td>
                  ...
                  <td></td>
                  <td>Subtotal</td>
                  <td><input type="text" name="subtotal" value="<?php echo $row['subtotal'];?>" id="subtotal" onkeyup="invoice(this)" autocomplete="off"></td>
            ...
            <td></td>
                  ...
                  <td><input type="text" name="salestax" value="<?php echo $row['salestax'];?>" id="salestax" onkeyup="invoice(this)" autocomplete="off"></td>
            ...
            <td></td>
                  ...
                  <td></td>
                  <td>Total</td>
                  <td><input type="text" name="totalbalance"  value="<?php echo $row['total_balance'];?>"id="totalbalance" onkeyup="invoice(this)" autocomplete="off"></td>
            ...
            <td></td>
                ...
                </tr> --><input type="hidden" name="gtotal" value="<?php echo $row['total'];?>"id="gtotal" autocomplete="off"><input type="hidden" name="invoice_id" value="<?php echo $id; ?>">

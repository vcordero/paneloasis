	...
	// Comparo el valor del captcha con la cookie
		...
		if ($data==NULL){
			...
			$sql=mysql_query($SQLSt);
	      	...
	      	if($sql){
		      	// Envio los datos del nuevo registro al admin
		      	        // old function     $answermailregister=sendemailadminregister($email,$phone);
		      	...
		      	// Envio los datos del nuevo registro al cliente
		      	//$answeruserregister=sendemailuserregister($email,$username,$pass);

                $to = $email; // Correo de registro del usuario.
            	$sub = "New Register";
                ...
                $answeruserregister=new_sendmail($to,$sub,$body,$attach='',$attach_as='',$cc='');
